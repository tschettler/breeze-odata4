...
import { EntityError, EntityErrorFromServer } from 'breeze-client/src/entity-manager';
 ...
 * Odata entity error
 */
export interface ODataEntityError extends EntityError, EntityErrorFromServer {
  /**
   * The OData error.
   */
  error: ODataError;
...
export abstract class OData4AjaxAdapter implements AjaxAdapter {
   ...
   */
  public prepareSaveErrors(saveContext: DataServiceSaveContext, failedResponses: Batch.FailedResponse[]): ODataEntityError[] {
    ...
    const result = failedResponses
      .map(failedResponse => {
        ...
        const error = Utilities.createError(failedResponse);
        const entityError: ODataEntityError = {
          entity: origEntity,
