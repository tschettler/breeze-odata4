import { AutoGeneratedKeyType, config, core, DataProperty, DataService, DataServiceSaveContext, DataServiceAdapter, DataType, Entity, EntityAspect, EntityQuery, EntityType, JsonResultsAdapter, MetadataStore, NodeContext, QueryContext, SaveResult, QueryResult, KeyMapping, VisitNodeResult, SaveBundle } from 'breeze-client';
...
import { batch } from "ts-odatajs/lib/odata";
import { EntityKey } from "breeze-client";
import { ChangeRequest, BatchRequest, ChangeRequestSet } from "ts-odatajs/lib/odata/batch";
...
export class ProxyServiceAdapter {
    ...
    public _createChangeRequestInterceptor(saveContext: DataServiceSaveContext, saveBundle: Object): {
        getRequest: (request: ChangeRequest, entity: Entity, index: number) => ChangeRequest;
...
export class OData4ServiceAdapter extends ProxyServiceAdapter implements DataServiceAdapter {
        ...
        return new Promise((resolve, reject) => {
            ...
            oData.read({
                ...
                (data: Edmx.Edmx, response: any) => {
                    ...
                    var csdlMetadata = this.metadata.dataServices;
                ...
                (error: Error) => {
                    var err = this.createError(error, url);
        ...
        return new Promise<SaveResult>((resolve, reject) => {
            oData.request({
                ...
                (data: batch.BatchResponse, response: any) => {
                    ...
                    var saveResult: SaveResult = { entities: entities, keyMappings: keyMappings, XHR: null };
                    data.__batchResponses.forEach((br: batch.ChangeResponseSet) => {
                        br.__changeResponses.forEach((cr: batch.ChangeResponse | batch.FailedResponse, index: number) => {
                            var response = (<batch.FailedResponse>cr).response || <batch.ChangeResponse>cr;
                            var statusCode = response.statusCode;
                            if ((!statusCode) || Number(statusCode) >= 400) {
                                var err = this.createError(cr, url);
                                reject(err);
                                return;
                            }
                            ...
                            var rawEntity: Entity = response.data;
                            if (rawEntity) {
                                ...
                                if (tempKey) {
                                    ...
                                    if (entityType.autoGeneratedKeyType !== AutoGeneratedKeyType.None) {
                                        ...
                                        var keyMapping: KeyMapping = { entityTypeName: entityType.name, tempValue: tempValue, realValue: realKey.values[0] };
                    ...
                    var error = this.createError(err, url);
                    reject(error);
    ...
    public JsonResultsAdapter: JsonResultsAdapter = new JsonResultsAdapter(
        {
            name: 'Test',
            visitNode: (node: any, mappingContext: QueryContext, nodeContext: NodeContext): VisitNodeResult => {
                var workingNode = node;
                var result: VisitNodeResult = {};
                if (node === null) {
                    return result;
                }
                ...
                if (nodeContext.nodeType === 'root') {
                    if (mappingContext.query) {
                        ...
                        var nodeODataType: string = node['@odata.type'];
                ...
                var et = entityTypeName && <EntityType>mappingContext.entityManager.metadataStore.getEntityType(entityTypeName, true);
    ...
    private createChangeRequests(saveContext: DataServiceSaveContext, saveBundle: SaveBundle): BatchRequest {
        var changeRequestInterceptor = this._createChangeRequestInterceptor(saveContext, saveBundle);
        var changeRequests: ChangeRequest[] = [];
        var tempKeys: EntityKey[] = [];
        var contentKeys: Entity[] = [];
        ...
        var changeRequestSet: ChangeRequestSet[] = [
            {
    ...
    private createError(error: any, url: string): ODataError {
        ...
        var response = error && (<batch.FailedResponse>error).response;
        if (!response) {
